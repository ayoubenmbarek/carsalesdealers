...
class CarsalesDealersSpider(scrapy.Spider):
        name = "carsalesdealersperdepartment1" #carsalesdealersperdepartment contain the cache of 09
        handle_httpstatus_list = [200, 302, 301]#, 500]#, 403]
        allowed_domains = ['carsales.com.au']
	    #sudo docker run -p 5023:5023 -p 8050:8050 -p 8051:8051 scrapinghub/splash
        #start_urls = ['https://www.carsales.com.au/dealer-services/dealers?s=900&sb=featured'] 
	...
	start_urls = ['https://www.carsales.com.au/dealer-services/dealers?s=7485&q=(And.Market.Car._.Status.1._.Department.Showroom.)&sb=dealername']#the last
	...
	#change separator awk 'gsub(",",";")' latest5_carsales_2017_09_v4.csv
        def parse(self, response):
            ...
            for list_dealers in container:
                ...
                myItem["AGENCE_ADRESSE"] = list_dealers.xpath('.//div[@class="col-xs-8"]/p[1]/text()').extract_first()
	        ...
	        #myItem["AGENCE_DEPARTEMENT"] = 'showroom'
                request = scrapy.Request(full_url, headers=headers, callback=self.second_page)
                request.meta['myItem'] = myItem
                yield request
            #for i in range(1,1000):#find number of pages(496 all) 525 from2,46 #stopped in 279, last 345
               ...
               # yield scrapy.Request(next, headers=headers, callback=self.parse)
